...
public:
	...
	void func_ex7(T lhs, S rhs)
	{
		...
		//typedef tpyeof(lhs * rhs) product_type;		// pre-c++11 'some' compilers
	...
	void ex9()
	{
		...
		typedef decltype((p->m_x))	m_x_with_parens_type;	// by const S*
	}
	...
	void ex11()
	{
		...
		typedef decltype(x* y) product_xy_type;
		auto a = x * y;

		typedef decltype(cx* cy) product_cxcy_type;			// result is prvalue
		auto b = cx * cy;

		typedef decltype(d1 < d2 ? d1 : d2) cond_type;		// result is lvalue. & is added
		auto c = d1 < d2 ? d1 : d2;

		typedef decltype(x < d2 ? x : d2) cond_type_mixed;		// promtion of x to double
		auto d = x < d2 ? x : d2;
	...
	void ex15()
	{
		...
		cout << &lambda << " " << &lambda2 << endl;		// the addresses are different
		cout << &lambda << " " << &lambda3 << endl;		// the addresses are same
	}
